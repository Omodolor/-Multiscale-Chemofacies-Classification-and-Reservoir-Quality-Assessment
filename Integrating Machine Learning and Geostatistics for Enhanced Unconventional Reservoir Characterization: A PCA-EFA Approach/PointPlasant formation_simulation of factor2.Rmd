---
title: "Conditional Simulation for Factor 2 Point Pleasant Formation"
Subtitle: ""
author: ""
...
```{r}
library(readr)
df <- read_csv("/mnt/vstor/CSE_MSE_RXF131/staging/sdle/geospatial/core_xrf_xrd/chapter_two/factor_scores_PP.csv")
#View(df)
...
#   struct = struct,
#   flag.noreduce = TRUE,
#   equal = c("M1V1=0.4", "M2V1=0.55"),  # Adjusted based on observed semivariance
#   
#   
...
# Calculate experimental omnidirectional variogram
vario.omni <- vario.calc(db.anamor, nlag = 5, lag = 0.5)
...
model.gaus.vario <- model.auto(
  ...
  struct = struct,
  flag.noreduce = TRUE,
  equal = c("M1V1=0.2", "M2V1=0.67"),  # Adjusted based on observed semivariance
...
The following map represents the mean of 100 realizations.  Theory states that if we ran an infinite number of realizations and calculated the mean, the result would be equivalent to the Kriged solution.  100 realizations are probably not enough to perfectly see this relationship, but you can get an idea.  One thing to do would be to calculate the statistics of mean map and check the mean, standard deviation, and variance.  Another thing to do would be to plot the contours from the Kriged map created earlier on top of the mean map created from the 10 realizations. The last part of the code chunk which is currently commented out, will plot the earlier Kriged contours on top of the current mean map from 100 realizations.  Uncomment the last part of the code and run the chunk.  What do you think? 

```{r warning = FALSE}
plot(
  SimGrid_mean_dir,
  title = "Mean of 100 Simulations",
  pos.legend = 1,
  cex = .7,
  asp = 1.05,
 xlim = c(x_min, x_max),  # Use Min and Max for Longitude
...
plot(
  SimGrid_mean_dir,
  title = "Mean of 25 Simulations Factor 2: Porosity & Diagenesis
",
  pos.legend = 1,
  cex = .7,
  asp = 1.05,
...
plot <- ggplot(df_masked_plot, aes(x1, x2)) +
  ...
  scale_fill_gradientn(colors = rev(hcl.colors(20, "RdYlBu")),
                       breaks = c(-1.39, -1.09, -0.79, -0.49, -0.19, 0.11, 0.41), 
                       limits = c(-1.39, 0.43)) +
  coord_fixed() +
  ...
  labs(title = paste("Mean of 100 Simulations Factor 2: Porosity & Diagenesis"),
       fill = "Mean Value",
       x = "Longitude",
       y = "Latitude") +
  theme_minimal() +
...
# Save the plot
ggsave(filename = "PP_Factor2.png", 
       plot = plot, 
       height = 6, 
       width = 16, 
       dpi = 300, 
...
```{r}
# Select the top 5 rows with the highest `Cum Oil`
top_5_cum_oil <- filtered_data_with_grid[order(-filtered_data_with_grid$`Cum Oil`), ][1:5, ]

# View the result
print(top_5_cum_oil)


# Select the top 5 rows with the highest `Cum Gas`
top_5_cum_gas <- filtered_data_with_grid[order(-filtered_data_with_grid$`Mean_Value`), ][1:5, ]
...
# Remove NA values for plotting
df_masked_plot <- df_masked %>% drop_na()
# Ensure the required data columns exist
if (!all(c("WH_LONG83", "WH_LAT83", "Cum Oil") %in% colnames(filtered_data))) {
  stop("The columns 'WH_LONG83', 'WH_LAT83', and 'Cum Oil' must exist in filtered_data.")
}

# Categorize points based on the Cum Oil ranges
filtered_data <- filtered_data %>%
  mutate(
    SizeCategory = case_when(
      `Cum Oil` < 5355.5 ~ "Small",
      `Cum Oil` >= 5355.5 & `Cum Oil` <= 61855.5 ~ "Medium",
      `Cum Oil` > 61855.5 ~ "Large"
    ),
    PointSize = case_when(
...
# Categorize points based on the Cum Gas ranges
filtered_data2 <- filtered_data %>%
  mutate(
    SizeCategory = case_when(
      `Cum Gas` < 2598002 ~ "Small",
      `Cum Gas` >= 2598002 & `Cum Gas` <= 7380076 ~ "Medium",
      `Cum Gas` > 7380076 ~ "Large"
    ),
    PointSize = case_when(
      SizeCategory == "Small" ~ 1,
      SizeCategory == "Medium" ~ 2,
...
# Plot using ggplot, overlaying hard data points from filtered_data with varying sizes
plot <- ggplot(df_masked_plot, aes(x1, x2)) +
  geom_tile(aes(fill = mean)) +  # Apply fill aesthetic here for tiles
  scale_fill_gradientn(colors = rev(hcl.colors(20, "RdYlBu")),
                      breaks = c(-1.39, -1.09, -0.79, -0.49, -0.19, 0.11, 0.41), 
  ...
  # Overlay hard data points from dbpoint
  geom_point(data = dbpoint, aes(x = WH_LONG83, y = WH_LAT83), color = "black", size = 1.5, pch = 20) +  # Adjust size and color as needed
  # Overlay filtered data with varying point sizes based on Cum Oil
  geom_point(data = filtered_data, aes(x = WH_LONG83, y = WH_LAT83, size = PointSize), 
             color = "green", alpha = 0.7) +
  scale_size_continuous(name = "Cum Oil Size",
                        breaks = c(1, 2, 3),
                        labels = c("Small", "Medium", "Large"),
                        range = c(0.5, 1.5)) +
  
  # # Overlay filtered data with varying point sizes based on Cum Gas
  geom_point(data = filtered_data2, aes(x = WH_LONG83, y = WH_LAT83, size = PointSize),
             color = "red", alpha = 0.7) +
  scale_size_continuous(name = "Cum Gas Size",
                        breaks = c(1, 2, 3),
                        labels = c("Small", "Medium", "Large"),
                        range = c(0.5, 1.5)) +
  
  labs(title = paste("Mean of 100 Simulations Factor 2 with Cumulative Oil and Gas Points"),
       fill = "Mean Value",
       x = "Longitude",
       y = "Latitude") +
  theme_minimal() +
...
# Save the plot
ggsave(filename = "PP_Factor2_CumOilGas.png", 
       plot = plot, 
       height = 6, 
       width = 16, 
       dpi = 300, 
...
draw.xvalid(db_without.db,mode=4,name.data="Factor2")
```
