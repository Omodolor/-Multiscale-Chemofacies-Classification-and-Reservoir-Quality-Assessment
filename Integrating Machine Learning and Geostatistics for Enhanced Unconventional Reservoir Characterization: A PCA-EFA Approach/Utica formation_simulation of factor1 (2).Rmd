---
title: "Conditional Simulation for Factor 2 Utica Formation"
Subtitle: ""
author: ""
...
model.gaus.vario <- model.auto(
  ...
  struct = struct,
  flag.noreduce = TRUE,
  equal = c("M1V1=0.2", "M2V1=0.8"),  # Adjusted based on observed semivariance
...
plot <- ggplot(df_masked_plot, aes(x1, x2)) +
  ...
  scale_fill_gradientn(colors = rev(hcl.colors(20, "RdYlBu")),
                       breaks = c(-0.87, -0.44, -0.01, 0.42, 0.85), 
                       limits = c(-0.87, 0.85)) +
  coord_fixed() +
...
# Save the plot
ggsave(filename = "Utica_Factor2.png", 
       plot = plot, 
       height = 6, 
       width = 16, 
       dpi = 300, 
