...
#Option 1 (simple + consistent with your elbow approach)
```{r}
# Load libraries
library(cluster)
library(factoextra)
...
# 1) Elbow (WSS) plot
# -------------------------
elbow_plot <- fviz_nbclust(data_pca, kmeans, method = "wss", k.max = 10) +
...
# 2) Silhouette plot
# -------------------------
sil_plot <- fviz_nbclust(data_pca, kmeans, method = "silhouette", k.max = 10) +
  labs(
...
# -------------------------
# 3) Print silhouette scores for each k
# -------------------------
sil_df <- sil_plot$data
names(sil_df) <- c("k", "avg_silhouette")
print(sil_df)

best_k <- sil_df$k[which.max(sil_df$avg_silhouette)]
cat("Best k by average silhouette width:", best_k, "\n")
