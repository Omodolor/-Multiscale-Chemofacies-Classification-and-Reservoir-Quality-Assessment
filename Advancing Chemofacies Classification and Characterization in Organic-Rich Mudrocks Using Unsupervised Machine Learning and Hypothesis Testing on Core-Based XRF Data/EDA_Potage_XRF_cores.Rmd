---
title: "Appendix A –Tracker Core Data Pre-Treatment and Exploration; Utica Data Set"
author: "Hope"
...
# Description
This example demonstrates reproducible data preprocessing and exploratory analysis of Potage XRF core datasets.  
It includes steps for data cleaning, integration, missing-value imputation, and initial exploratory visualization in R.  
The workflow reflects the structured approach used in my research to prepare geochemical datasets for subsequent multivariate and factor analysis.



# To Checks the current print precision (number of digits shown) and Sets it to 15 so that numbers are displayed with more detail in the console
...
# Load the  the raw xrf data from kemeh, Potage Data 
```{r}


Raw_Potage_data <- read_csv(
  "/mnt/vstor/CSE_MSE_RXF131/staging/sdle/geospatial/core_xrf_xrd/XRF_two_core_in1/EDA_CORES/XRF_Potage.csv"
)
...
for (column_name in names(Raw_Potage_data)) {
  # Check if the column has any zero values
  if (any(Raw_Potage_data[[column_name]] == 0)) {
...
names(elements_percentage) <- sub("ppm", "%", names(elements_percentage))

# Print the converted values
print(elements_percentage)
...
for (col_name in names(values)) {
  Raw_Potage_data[[col_name]][Raw_Potage_data[[col_name]] == 0] <- values[col_name]
}

# Print the updated data frame
print(Raw_Potage_data)
...
Raw_Potage_data$'K_wt%' <- replace_zeros_with_avg(Raw_Potage_data$'K_wt%')
Raw_Potage_data$'Fe_wt%' <- replace_zeros_with_avg(Raw_Potage_data$'Fe_wt%')
Raw_Potage_data$'U_ppm' <- replace_zeros_with_avg(Raw_Potage_data$'U_ppm')
Raw_Potage_data$'Th_ppm' <- replace_zeros_with_avg(Raw_Potage_data$'Th_ppm')
...
for (col_name in names(values)) {
  if (col_name %in% names(Raw_Potage_data)) {
...
Raw_Potage_data$Formation <- NA

# Update 'Formation' column based on depth ranges
Raw_Potage_data$Formation[Raw_Potage_data$Depth_ft >= 6141.0 &
                            Raw_Potage_data$Depth_ft <= 6335] <- "Utica FM"
Raw_Potage_data$Formation[Raw_Potage_data$Depth_ft > 6335 &
                            Raw_Potage_data$Depth_ft <= 6444] <- "Point Pleasant FM"
Raw_Potage_data$Formation[Raw_Potage_data$Depth_ft > 6444 &
                            Raw_Potage_data$Depth_ft <= 6474] <- "Trenton"


# View the updated DataFrame
head(Raw_Potage_data)
names(Raw_Potage_data)
# That are data that hase formation toped picked
write.csv(Raw_Potage_data, "Part_clean_Potage.csv")
...
Potage_data_summary <- Raw_Potage_data[, !names(Raw_Potage_data) %in% c("Depth_ft", "Formation", "Chemofacies")] %>%
...
Raw_Potage_data <- Raw_Potage_data[, !names(Raw_Potage_data) %in% c("Depth_ft", "Formation", "Chemofacies")]

# Standardize the data
Potage_standardized <- scale(Raw_Potage_data)

# Kaiser-Meyer-Olkin (KMO) Test
Potage_kmo_result <- KMO(Potage_standardized)

# Bartlett's Test of Sphericity
Potage_bartlett_result <- cortest.bartlett(Potage_standardized)

# Print the results
print(Potage_kmo_result)  # A KMO value closer to 1 suggests better suitability for PCA
print(Potage_bartlett_result)  # For Bartlett's test, a significant p-value (< 0.05) indicates suitability for PCA
```
...
#This R code is creating a combined boxplot to visualize the distribution of multiple variables in the Raw_Potage_data DataFrame. Here's a simple explanation of what each part does:


```{r}
# Assuming your data frame is named df
# Reshaping the data for plotting
Potage_long_df <- gather(Raw_Potage_data, key = "Variable", value = "Value")

# Creating individual box plots for each variable
ggplot(Potage_long_df, aes(x = Variable, y = Value)) +
  geom_boxplot() +
...
outliers_by_column <- calculate_iqr_outliers(Raw_Potage_data)
...
the The Zn_ppm value 1622.14ppm and 275.49ppm would be replace by the average the vales above and below it.
...
for_Potage_replace_with_neighbor_mean <- function(dataframe, column, value) {
...
# Replace the two highest values in 'Zn_ppm'
Raw_Potage_data <- for_Potage_replace_with_neighbor_mean(Raw_Potage_data, 'Zn_ppm', 1622.14)
Raw_Potage_data <- for_Potage_replace_with_neighbor_mean(Raw_Potage_data, 'Zn_ppm', 275.49)

# Rename the dataframe to no_out
Clean_Potage_datanoout <- Raw_Potage_data

# Optionally, view the modified data for verification
print(Clean_Potage_datanoout[which(Clean_Potage_datanoout$Zn_ppm %in% c(1622.14, 275.49)), 'Zn_ppm'])

write.csv(Clean_Potage_datanoout, "Clean_Potage_datanoout.csv")
...
Potage_long_df2 <- gather(Clean_Potage_datanoout, key = "Variable", value = "Value")

# Creating individual box plots for each variable
ggplot(Potage_long_df2, aes(x = Variable, y = Value)) +
  geom_boxplot() +
  theme(axis.text.x = element_text(
    angle = 90,
    vjust = 0.5,
...
Calculating correlation matrix between variables in data after outlier remover
```{r}

# Calculating correlation matrix
#Exclude 'Depth_ft' and 'Formation' columns
potage_corr_matrix <- cor(Clean_Potage_datanoout[, !names(Clean_Potage_datanoout) %in% c("Depth_ft", "Formation", "Chemofacies")])

#[, sapply(data, is.numeric)])  # Select only numeric columns

# Melting the correlation matrix
potage_corr_melted <- melt(potage_corr_matrix)

ggplot(potage_corr_melted, aes(Var1, Var2, fill = value)) +
  geom_tile() +
