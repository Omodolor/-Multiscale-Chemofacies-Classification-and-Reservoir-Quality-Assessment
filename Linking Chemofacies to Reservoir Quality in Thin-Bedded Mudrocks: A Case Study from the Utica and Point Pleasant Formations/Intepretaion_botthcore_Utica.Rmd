---
title: "Interpretation of both core for the Utica formation after processing Data Set"
...
```{r}
Potage_Utica_Aligned_XRF_Geochemical_Merged <- read_csv("/mnt/vstor/CSE_MSE_RXF131/staging/sdle/geospatial/core_xrf_xrd/XRF_two_core_in1/chaper_3/Interpretation_both_core/Potage_Utica_Aligned_XRF_Geochemical_Merged1.csv")

names(Potage_Utica_Aligned_XRF_Geochemical_Merged)
...
Washe_Utica_Aligned_XRF_Geochemical_Merged <- read_csv("/mnt/vstor/CSE_MSE_RXF131/staging/sdle/geospatial/core_xrf_xrd/XRF_two_core_in1/chaper_3/Interpretation_both_core/Washe_Utica_Aligned_XRF_Geochemical_Merged.csv")

names(Washe_Utica_Aligned_XRF_Geochemical_Merged)


library(dplyr)

# Step 1: Rename consistent columns in Washe (Utica)
Washe_Utica_Aligned_XRF_Geochemical_Merged <- Washe_Utica_Aligned_XRF_Geochemical_Merged %>%
  rename(
    `TOC_(wt.%)` = `TOC (wt.%)`,
    Total_Porosity = Porosity,
    `K-Feldspar` = `K_Feldspar`
    # Leave Dolomite_Fe_Dolomite unchanged if already named that
  )
...
Washe_Utica_Aligned_XRF_Geochemical_Merged[missing_cols] <- NA

# Step 3: Add Dolomite_Fe_Dolomite to Potage if it’s missing
if (!"Dolomite_Fe_Dolomite" %in% names(Potage_Utica_Aligned_XRF_Geochemical_Merged)) {
  Potage_Utica_Aligned_XRF_Geochemical_Merged$Dolomite_Fe_Dolomite <- NA
}

# Step 4: Reorder Washe columns to match Potage for consistent bind
Washe_Utica_Aligned_XRF_Geochemical_Merged <- Washe_Utica_Aligned_XRF_Geochemical_Merged %>%
  dplyr::select(all_of(names(Potage_Utica_Aligned_XRF_Geochemical_Merged)))

# Step 5: Add Core identifier and combine the two Utica datasets
Utica_combined_df <- bind_rows(
  Potage_Utica_Aligned_XRF_Geochemical_Merged %>% mutate(Core = "Potage"),
  Washe_Utica_Aligned_XRF_Geochemical_Merged %>% mutate(Core = "Washe")
)
...
# Utica_Bivariate_Chemofacies_BoldAxesAndLines
```{r}
library(ggplot2)
library(patchwork)
...
# Plot 1: Si vs Al
p1 <- ggplot(Utica_combined_df, aes(x = `Al_wt%`, y = `Si_wt%`, color = factor(chemofacies))) +
  geom_point(size = 2, alpha = 0.8) +
  scale_color_manual(values = facies_colors) +
  geom_hline(yintercept = 29, linetype = "dashed") +
  labs(title = "Si vs Al", x = "Al (wt%)", y = "Si (wt%)") +
  custom_theme

# Plot 2: Ni vs Al
p2 <- ggplot(Utica_combined_df, aes(x = `Al_wt%`, y = `Ni_ppm`, color = factor(chemofacies))) +
  geom_point(size = 2, alpha = 0.8) +
  scale_color_manual(values = facies_colors) +
  geom_hline(yintercept = 122, linetype = "dashed") +
  labs(title = "Ni vs Al", x = "Al (wt%)", y = "Ni (ppm)") +
  custom_theme

# Plot 3: Mo vs Al
p3 <- ggplot(Utica_combined_df, aes(x = `Al_wt%`, y = `Mo_ppm`, color = factor(chemofacies))) +
  geom_point(size = 2, alpha = 0.8) +
  scale_color_manual(values = facies_colors) +
  geom_hline(yintercept = 10, linetype = "dashed") +
  labs(title = "Mo vs Al", x = "Al (wt%)", y = "Mo (ppm)") +
  custom_theme

# Plot 4: Mg vs Ca
p4 <- ggplot(Utica_combined_df, aes(x = `Ca_wt%`, y = `Mg_wt%`, color = factor(chemofacies))) +
  geom_point(size = 2, alpha = 0.8) +
  scale_color_manual(values = facies_colors) +
  geom_hline(yintercept = 1.8, linetype = "dashed") +
  labs(title = "Mg vs Ca", x = "Ca (wt%)", y = "Mg (wt%)") +
  custom_theme

# Combine plots in 2x2 layout
final_plot <- (p1 | p2) / (p3 | p4) + plot_annotation(tag_levels = 'A')

final_plot

# Save the plot
ggsave(
  filename = "Utica_Bivariate_Chemofacies_BoldAxesAndLines.png",
  plot = final_plot,
  width = 12,
  height = 10,
  dpi = 600,
...
create_plot <- function(element, element_label, facies_colors) {
  ggplot(Utica_combined_df, aes(x = factor(chemofacies), y = .data[[element]], fill = factor(chemofacies))) +
...
for (element in names(elements_to_plot)) {
  ...
  print(plot)
  ggsave(paste0("Utica_presentation2_plot_", gsub(" ", "_", label), ".png"),
...
# Utica_Boxplot_Chemofacies_BoldAxesAndLines
```{r}
library(ggplot2)
library(patchwork)
library(dplyr)
...
p11 <- ggplot(filter(Utica_combined_df, is.finite(Total_Porosity)), 
...
# Plot 2: TOC
p22 <- ggplot(filter(Utica_combined_df, is.finite(`TOC_(wt.%)`)), 
              aes(x = factor(chemofacies), y = `TOC_(wt.%)`, fill = factor(chemofacies))) +
  geom_boxplot(outlier.color = "red", outlier.shape = 3, color = "black") +
  stat_boxplot(geom = "errorbar", width = 0.25, color = "black") +
  scale_fill_manual(values = facies_colors) +
  labs(title = "TOC", x = "Chemofacies", y = "TOC (wt)") +
  custom_theme

# Plot 3: Total Clay
p33 <- ggplot(filter(Utica_combined_df, is.finite(Total_Clay)), 
...
# Plot 4: MBI
p44 <- ggplot(filter(Utica_combined_df, is.finite(MBI)), 
...
# Combine plots in 2x2 layout
final_plot <- (p11 | p22) / (p33 | p44) + plot_annotation(tag_levels = 'A')
final_plot

# Save the combined plot
ggsave(
  filename = "Utica_Boxplot_Chemofacies_BoldAxesAndLines.png",
  plot = final_plot,
  width = 12,
  height = 10,
  dpi = 600,
...
# Utica_Factor_Scores_Boxplot_2x2
```{r}
library(ggplot2)
library(patchwork)
library(dplyr)
...
p1 <- ggplot(filter(Utica_combined_df, is.finite(Factor1)), 
...
p2 <- ggplot(filter(Utica_combined_df, is.finite(Factor2)), 
...
p3 <- ggplot(filter(Utica_combined_df, is.finite(Factor3)), 
...
# Save to file
ggsave(
  filename = "Utica_Factor_Scores_Boxplot_2x2.png",
  plot = final_plot,
  width = 16,
  height = 12,
  dpi = 600,
